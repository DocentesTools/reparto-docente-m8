...
- **Rendered reparto documents use an explicit three-locale catalog (C14).**
  Internal-draft, school-leadership, teacher-summary, and final documents now
  translate service-owned vocabulary and document-only enum labels in English,
  Spanish, or French from the locale persisted by C13. The pure renderer takes
  an injected, revision-labelled catalog rather than reading request state;
  plural sentences use the locale's gettext plural rule. Twelve reviewed golden
  artifacts (four document types by three locales) pin deterministic bytes.
  Generated plan-staleness reasons now persist an internal stable code and
  language-neutral parameters alongside their English compatibility fallback,
  while historical and user-authored reasons remain verbatim. The new metadata
...
- **A stored export records the language it was requested under (C13).**
  ...
  falling back to the locale negotiated from `Accept-Language` and then to
  English, and `ExportArtifactPublic` always answers with it. The value is a
  new `NOT NULL DEFAULT 'EN'` column on `reparto_export_artifact`, so the
  Compose bootstrap's autogenerated revision backfills every existing row to
  English — the language it was rendered in — and a second boot detects no
  drift. `DocumentRenderingService.render` takes its locale-dependent catalog
  as an explicit argument (never the request `ContextVar`); C14 supplies the
  translated document catalog, while JSON/CSV bytes remain language-neutral
  and byte-identical across locales. `checksum` remains
  `sha256(content)`, never salted with the locale. The response declares
  `Content-Language` (the persisted locale) and `Vary: Accept-Language`;
