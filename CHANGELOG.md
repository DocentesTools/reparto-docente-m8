...
- **A stored export records the language it was requested under (C13).**
  ...
  drift. `DocumentRenderingService.render` takes the locale as an explicit
  argument (never the request `ContextVar`); until the document catalog lands
  every locale still renders the same English text, and JSON/CSV bytes are
  language-neutral and byte-identical across locales. `checksum` remains
